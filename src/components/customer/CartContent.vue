<template>
  <div>
      ...
      <v-menu activator="parent" offset-y>
        ...
        <v-list class="dropdown-menu ml-auto" width="200">
          ...
          <v-list-item @click="clearAllProducts" class="text-red">
            <v-list-item-title><v-icon>mdi-delete-empty-outline</v-icon> Clear cart</v-list-item-title>
          </v-list-item>
        </v-list>
      </v-menu>
    </v-row>
    ...
    <v-container fluid class="px-0 d-flex flex-column">
      ...
      <v-row v-if="cartProducts.length === 0" class="empty-holder justify-center align-center">
        <LottieAnimation :animationData="animationData" width="200px" height="200px" />
        <span class="empty-title my-2">Your cart is empty.</span>
        <v-btn variant="text">
          <router-link color="primary" class="back-menu-link" to="/">Go back to menu to
            order</router-link>
        </v-btn>
      </v-row>
      ...
      <v-card v-if="cartProducts.length > 0" class="bottom-section action-cards w-100" elevation="10">
        <v-col>
          <v-row align="center" justify="space-between">
            ...
            <v-col>
              <p class="font-weight-bold text-right">₱ {{ calculateTotal(cartProducts) }}</p>
            </v-col>
          </v-row>
        </v-col>
